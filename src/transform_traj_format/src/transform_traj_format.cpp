...
int main(int argc, char **argv)
{
    ...
    std::string output_traj_file;
    n.param("input_traj_file", input_traj_file, std::string("in"));
    n.param("output_traj_file", output_traj_file, std::string("out"));
    std::cout<<"input_traj_file: "<<input_traj_file<<std::endl;
    std::cout<<"output_traj_file: "<<output_traj_file<<std::endl;

//    std::ifstream tf("/home/lyh/lab/data/calibrate_0907/0906_gaolou.txt",std::ios::in);
//    std::ifstream tf("/home/lyh/lab/data/zhoushan_1225/data1/1225_01.txt",std::ios::in);
//    std::ofstream of("/home/lyh/lab/data/zhoushan_1225/data1/transformed_format.csv",std::ios::ate);

    std::ifstream tf(input_traj_file,std::ios::in);
    std::ofstream of(output_traj_file,std::ios::ate);
    ...
    while(!tf.eof())
    {
        ...
        line_cnt++;
//        std::cout<<"timestamp = "<<std::to_string(dsecond*1e9)<<" , x y x = "<<dx<<" "<<dy<<" "<<dz<<" , r p y = "<<droll<<" "<<dpitch<<" "<<dyaw<<std::endl;

        Eigen::Matrix3d R_M = Eigen::AngleAxisd(-1*dyaw*M_PI/180.0,Eigen::Vector3d(0,0,1)).matrix()*Eigen::AngleAxisd(dpitch*M_PI/180.0,Eigen::Vector3d(1,0,0)).matrix()*Eigen::AngleAxisd(droll*M_PI/180.0,Eigen::Vector3d(0,1,0)).matrix();

        Eigen::Quaterniond q(R_M);
//        std::cout<<"timestamp = "<<std::setw(14)<<std::setprecision(0)<<dsecond*1e9<<std::setiosflags(std::ios::fixed)<<std::setprecision(9)<<" , x y x = "<<dx<<" "<<dy<<" "<<dz<<" , w x y z = "<<q.coeffs().w()<<" "<<q.coeffs().x()<<" "<<q.coeffs().y()<<" "<<q.coeffs().z()<<std::endl;
//        of<<std::setw(14)<<std::setprecision(0)<<dsecond*1e9<<std::setiosflags(std::ios::fixed)<<std::setprecision(9)<<", ,"<<dx<<","<<dy<<","<<dz<<","<<q.coeffs().w()<<","<<q.coeffs().x()<<","<<q.coeffs().y()<<","<<q.coeffs().z()<<std::endl;
        of<<std::setw(14)<<std::setprecision(0)<<std::setiosflags(std::ios::fixed)<<dsecond*1e9<<std::setprecision(9)<<", ,"<<dx<<","<<dy<<","<<dz<<","<<q.coeffs().w()<<","<<q.coeffs().x()<<","<<q.coeffs().y()<<","<<q.coeffs().z()<<std::endl;

...
bool load_lidar_and_combine(const std::string& bag_path,const std::string& output_save_path,int save_per_point,const std::string& rot_trans_str,double time_offset)
{
    ...
    for (const rosbag::MessageInstance& m : view)
    {
        ...
        for(int i = 0;i< pointcloud.points.size();i+=1)
        {
            ...
            //std::cout<<int(timestamp*1000)%1000<<std::endl;
            //std::cout<<int(timestamp*1e6)%1000<<std::endl;
            
            //std::cout<<pos_table[int(timestamp)][999].h<<std::endl;
            		
            //POS cur_pos111 = pos_table[int(timestamp)][1000];
            

            //extract the pose transform of each point
            POS cur_pos = pos_table[int(timestamp)][int(timestamp*1000)%1000];
            
            int last_pos_index = int(timestamp*1000)%1000 - 1;
            int last_timestamp = timestamp;
            double slerp_t = (int(timestamp*1e6)%1000)/1000.0;
            //std::cout<<slerp_t<<std::endl;
            if(last_pos_index == -1)
            {
            	last_pos_index = 999;
            ...
            //std::cout<<last_pos_index<<std::endl;
            		
            POS last_pos = pos_table[last_timestamp][last_pos_index];

            Eigen::Matrix3d R_M_cur = Eigen::AngleAxisd(-1*cur_pos.h*M_PI/180.0,Eigen::Vector3d(0,0,1)).matrix()*Eigen::AngleAxisd(1*cur_pos.p*M_PI/180.0,Eigen::Vector3d(1,0,0)).matrix()*Eigen::AngleAxisd(cur_pos.r*M_PI/180.0,Eigen::Vector3d(0,1,0)).matrix();
            Eigen::Matrix3d R_M_last = Eigen::AngleAxisd(-1*last_pos.h*M_PI/180.0,Eigen::Vector3d(0,0,1)).matrix()*Eigen::AngleAxisd(1*last_pos.p*M_PI/180.0,Eigen::Vector3d(1,0,0)).matrix()*Eigen::AngleAxisd(last_pos.r*M_PI/180.0,Eigen::Vector3d(0,1,0)).matrix();
            
            Eigen::Quaterniond R_M_cur_quat(R_M_cur);
            Eigen::Quaterniond R_M_last_quat(R_M_last);
            //R_M_last.slerp(0.5,R_M_last)
            
            Eigen::Matrix3d R_M = R_M_last_quat.slerp(slerp_t,R_M_cur_quat).toRotationMatrix();
            
            
            Eigen::Isometry3d Tw = Eigen::Isometry3d::Identity();
            
            double pretrans_x = (cur_pos.x - last_pos.x)*slerp_t + last_pos.x;
            double pretrans_y = (cur_pos.y - last_pos.y)*slerp_t + last_pos.y;
            double pretrans_z = (cur_pos.z - last_pos.z)*slerp_t + last_pos.z;
            
            Tw.rotate(R_M);
            
            Tw.pretranslate(Eigen::Vector3d(pretrans_x,pretrans_y,pretrans_z));
